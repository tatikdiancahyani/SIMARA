...
@section('title',"Jadwal Rapat: ${judul}")
...
<form action="{{ route('rapat') }}" method="GET"
                ...
                <tr>
                    ...
                    <td>
                        @if($jadwalRapat->beritaAcara)
                            ...
                            <strong>Hasil:</strong>
                            {{ Str::limit($jadwalRapat->beritaAcara->hasil_rapat, 1000) }}
                            <hr />
                            <button type="button" onclick="bukaModal('{{ $jadwalRapat->id_jadwal }}')"
                                class="button create-button"> Ubah Notulen </button>
                        @else
                            <button type="button" onclick="bukaModal('{{ $jadwalRapat->id_jadwal }}')"
                                class="button create-button"> Input Notulen </button>
                        @endif
                        ...
                        <div id="modal-{{ $jadwalRapat->id_jadwal }}" class="modal" style="display: none;">
                            <div class="modal-content">
                                <div>
                                    <span class="close"
                                        onclick="tutupModal('{{ $jadwalRapat->id_jadwal }}')">&times;</span>
                                </div>
                                <h1> Notulen</h1>
                                <form action="{{ route('store.berita') }}" method="POST">
                                    ...
                                    <textarea id="hasil_rapat" name="hasil_rapat" class="form-control"
                                        placeholder="Masukkan hasil rapat">{{ $jadwalRapat->beritaAcara?->hasil_rapat }}</textarea>
